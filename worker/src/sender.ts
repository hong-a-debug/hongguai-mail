...
// ============================================================
// ✅ 已修改：发信接口 - 强制启用，绕过 token 验证
// ============================================================
api.post('/send', async (c) => {
  // 强制使用 Resend 通道，忽略配置检查
  const sendChannel = 'resend';
  
  // 检查发件人邮箱是否配置
  if (!c.env.SENDER_EMAIL) {
    return c.json({ code: 'SEND_UNAVAILABLE', message: 'Sender email not configured' }, 503);
  }

  // 检查 Resend API Key
  if (!c.env.RESEND_API_KEY) {
    return c.json({ code: 'SEND_UNAVAILABLE', message: 'Resend API Key not configured' }, 503);
  }

  // ========== 临时绕过 token 验证，使用固定邮箱 ==========
  ...
  try {
    ...
    if (!response.ok) {
      ...
      return c.json({ code: 'SEND_PROVIDER_ERROR', message: `Email provider rejected: ${errorText}` }, 502);
    }
...
app.get('/config', (c) => {
  ...
  const sendChannel = 'resend'; // 强制设置
  const enabledSenders = ['resend'];
